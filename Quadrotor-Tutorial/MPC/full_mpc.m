...
nlobj.MV(1).Max = 2*m*g;
nlobj.MV(2).Min = -0.5;    % Roll torque
nlobj.MV(2).Max = 0.5;
nlobj.MV(3).Min = -0.5;    % Pitch torque
nlobj.MV(3).Max = 0.5;
nlobj.MV(4).Min = -0.5;  % Yaw torque
nlobj.MV(4).Max = 0.5;

nlobj.States(4).Min = -45;  % phi atti lb
nlobj.States(4).Max = 45;   % phi atti ub

nlobj.States(5).Min = -45;   % theta atti  lb
nlobj.States(5).Max = 45;    % theta atti ub
...
ref=[10*ones(length(time),1) 25*ones(length(time),1) 15*ones(length(time),1)];
...
plot(time, xHistory(:,2), 'LineWidth', 1.5); ylabel('y (m)'); title('Position Y');legend('y^{ref}','y'); grid on;

subplot(3,3,3);
plot(time, ref_full(:,3), 'LineWidth', 1.5); hold on;
plot(time, xHistory(:,3), 'LineWidth', 1.5); ylabel('z (m)'); title('Position Z');legend('z^{ref}','z'); grid on;
